...
<div class="app-application-form">
    ...
    <?= $form->field($model, 'intake_code')->widget(Select2::classname(), [
        'initValueText' => ($model->intake_code && $model->applicationIntake) ? $model->applicationIntake->intake_name : '',
        'options' => ['placeholder' => 'Search for an intake ...'],
        'pluginOptions' => [
            'allowClear' => true,
            'minimumInputLength' => 1,
            'ajax' => [
                'url' => Url::to(['application-intake/application-intake-list']),
                'dataType' => 'json',
                'data' => new \yii\web\JsExpression('function(params) { return {q:params.term}; }')
            ],
            'escapeMarkup' => new \yii\web\JsExpression('function (markup) { return markup; }'),
    ...
    <?= $form->field($model, 'app_publish')->textInput() ?>

    <?= $form->field($model, 'letter_downloaded')->textInput() ?>
